...
class Npc(GSprite):
    ...
    def load(self, id: str, info: dict):
        try:
            ...
            self.info = self.db.getOne("SELECT * FROM personaggi WHERE id = " + str(npg["personaggio_id"]))
            razza = self.db.getOne("SELECT * FROM razze WHERE id = " + str(self.info["razza_id"]))
            self.razza = razza
            dimension = razza["dimensione"] * state.config["tile_size"]
            self.state = self.db.getOne("SELECT * FROM stato WHERE personaggio_id = " + str(self.info["id"]))
            ...
            self.rect = self.image.get_rect(center=(info["pos"][0] * (dimension // 2), info["pos"][1] * (dimension // 2)))
